...
!-------------------------------------------------------------------------------
  ...
  !TEMPORARY VARIABLES
    ...
    write(iulog,*)'Outputting to file: ', ncid, this%hist_filename
  ...
  subroutine hist_write(this, bounds, numf, filter, time_vars, velocity)
    ...
    associate(                                                   &
      ...
      !Jing Tao: Commenting out this means writing every time step into the output file.
...
!    if (associated(col_ws%h2osoi_liq)) continue
  ...
  integer :: npft_loc
  cc = 1
  !write(*,*)'TJ In subroutine BeTRSimulationSetBiophysForcing', bounds%begc, bounds%endc
  ! Note this%t_soi(time, level)
  do c = bounds%begc, bounds%endc
    if(.not. this%active_col(c))cycle
...
!  type(waterstate_type), intent(in) :: waterstate_vars
  ...
  do jj = 1, num_flux1d
    if(betr_offline)then
      ...
      !write(*,*) 'hist_create_fluxes, checking 1dvarname:', ncid, this%flux_hist1d_var(jj)%use_default, this%flux_hist1d_var(jj)%varname, this%flux_hist1d_var(jj)%long_name 
  ...
  !-----------------------
  !write(*,*) 'num_flux2d:',num_flux2d !24
  !do jj = 1, num_flux2d
  ...
  !write(*,*) 'num_flux1d:',num_flux1d !395
  !do jj = 1, num_flux1d
  !  do fc = 1, numf
  !    this%hist_fluxes_1d_accum(c,jj) = this%hist_fluxes_1d_accum(c,jj)/this%hist_naccum
  !  enddo
  !  write(*,*)'Outputting 1d varible:',ncid,this%flux_hist1d_var(jj)%varname
  !  if(trim(this%flux_hist1d_var(jj)%use_default)/='inactive')then
  !    data1dptr => this%hist_fluxes_1d_accum(begc:endc, jj)
      !write(*,*) 'before putvar', this%flux_hist1d_var(jj)%use_default, this%flux_hist1d_var(jj)%varname
